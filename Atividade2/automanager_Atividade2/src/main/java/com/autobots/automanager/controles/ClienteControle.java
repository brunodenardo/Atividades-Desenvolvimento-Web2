...
public class ClienteControle {
	...
	public ResponseEntity<Cliente> obterCliente(@PathVariable long id) {
		...
		if (cliente == null) {
			ResponseEntity<Cliente> resposta = new ResponseEntity<>(HttpStatus.BAD_REQUEST);
			return resposta;
		} else {
	...
	public ResponseEntity<List<Cliente>> obterClientes() {
		List<Cliente> clientes = repositorio.findAll();
		if (clientes.isEmpty()) {
			ResponseEntity<List<Cliente>> resposta = new ResponseEntity<>(HttpStatus.BAD_REQUEST);
			return resposta;
		} else {
